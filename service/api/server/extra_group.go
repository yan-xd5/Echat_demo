...
func (s *extraGroupImpl) KickMember(ctx context.Context) {
	...
	if target != nil && target.Role == entity.RoleOwner {
		writeJSON(w, 403, map[string]interface{}{"code": 1, "message": "不能踢出群主"})
		return
	}
	...
	writeJSON(w, 200, map[string]interface{}{"code": 0, "message": "已踢出"})
}
...
func (s *extraGroupImpl) MuteMember(ctx context.Context) {
	...
	writeJSON(w, 200, map[string]interface{}{"code": 0, "message": "已禁言", "ban_id": banID})
}
...
func (s *extraGroupImpl) UnmuteMember(ctx context.Context) {
	...
	writeJSON(w, 200, map[string]interface{}{"code": 0, "message": "已解除禁言"})
}
...
func (s *extraGroupImpl) UpdateMemberRole(ctx context.Context) {
	...
	if op == nil || op.Role != entity.RoleOwner {
		writeJSON(w, 403, map[string]interface{}{"code": 1, "message": "只有群主能修改角色"})
		return
	}
	role := entity.Role(req.Role)
	if role != entity.RoleAdmin && role != entity.RoleMember {
		writeJSON(w, 400, map[string]interface{}{"code": 1, "message": "无效角色"})
		return
	}
	...
	writeJSON(w, 200, map[string]interface{}{"code": 0, "message": "角色已修改"})
}
...
func (s *extraGroupImpl) GetGroupRequests(ctx context.Context) {
	...
	gid := body.Gid
	if gid == "" {
		writeJSON(w, 400, map[string]interface{}{"code": 1, "message": "缺少 gid"})
		return
	}
	// 检查是否是管理员
	op, _ := s.groupRepo.FindMember(ctx, gid, uid)
	if op == nil || (op.Role != entity.RoleOwner && op.Role != entity.RoleAdmin) {
		writeJSON(w, 403, map[string]interface{}{"code": 1, "message": "无权查看"})
		return
	}
...
func (s *extraGroupImpl) ApproveGroupRequest(ctx context.Context) {
	...
	if err != nil || gr == nil {
		writeJSON(w, 400, map[string]interface{}{"code": 1, "message": "申请不存在"})
		return
	}
	// 检查是否是管理员
	...
	if req.Approve {
		...
		if err := s.groupRepo.SaveMember(ctx, &entity.GroupMember{
			...
			writeJSON(w, 500, map[string]interface{}{"code": 999, "message": "添加成员失败: " + err.Error()})
			return
		}
	...
	writeJSON(w, 200, map[string]interface{}{"code": 0, "message": "已处理"})
}
